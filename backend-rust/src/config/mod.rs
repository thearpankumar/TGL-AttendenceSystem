    ...
    pub fn from_env() -> anyhow::Result<Self> {
        ...
        Ok(AppConfig {
            ...
            storage: StorageConfig {
                ...
                s3: {
                    let bucket = env::var("AWS_S3_BUCKET").unwrap_or_else(|_| "test-bucket".to_string());
                    let access_key_id = env::var("AWS_ACCESS_KEY_ID").unwrap_or_else(|_| "test-key".to_string());
                    let secret_access_key = env::var("AWS_SECRET_ACCESS_KEY").unwrap_or_else(|_| "test-secret".to_string());
...
impl Default for AppConfig {
    fn default() -> Self {
        ...
        Self::from_env().unwrap_or_else(|_| AppConfig {
            ...
            mongodb_max_pool_size: env::var("MONGODB_MAX_POOL_SIZE").ok().and_then(|p| p.parse().ok()).unwrap_or(50),
            mongodb_min_pool_size: env::var("MONGODB_MIN_POOL_SIZE").ok().and_then(|p| p.parse().ok()).unwrap_or(5),
            jwt_secret: env::var("JWT_SECRET").unwrap_or_else(|_| "dev-secret-change-in-production".to_string()),
            jwt_expire: env::var("JWT_EXPIRE").unwrap_or_else(|_| "7d".to_string()),
            admin_secret: env::var("ADMIN_SECRET").unwrap_or_else(|_| "dev-admin-secret".to_string()),
            ...
            storage: StorageConfig {
                ...
                s3: S3Config {
                    ...
                    region: env::var("AWS_REGION").unwrap_or_else(|_| "us-east-1".to_string()),
                    access_key_id: env::var("AWS_ACCESS_KEY_ID").unwrap_or_else(|_| "test-key".to_string()),
                    secret_access_key: env::var("AWS_SECRET_ACCESS_KEY").unwrap_or_else(|_| "test-secret".to_string()),
                },
            },
            redis: RedisConfig {
                url: env::var("REDIS_URL").unwrap_or_default(),
            ...
            webauthn: WebAuthnConfig {
                rp_name: env::var("WEBAUTHN_RP_NAME").unwrap_or_else(|_| "Attendix Attendance System".to_string()),
                rp_id: env::var("WEBAUTHN_RP_ID").unwrap_or_else(|_| "localhost".to_string()),
                origin: env::var("WEBAUTHN_ORIGIN").unwrap_or_else(|_| "http://localhost:5000".to_string()),
            },
        })
    }
}
