...
pub async fn get_dashboard_stats(
    ...
    let timeframe_range = if let (Some(start_str), Some(end_str)) = (&query.start_date, &query.end_date) {
    ...
    if let Some((start, end)) = &timeframe_range {
        session_filter.insert("createdAt", doc! {
            "$gte": start,
            "$lte": end
        });
    }
    ...
    let mut session_batch_map: std::collections::HashMap<mongodb::bson::oid::ObjectId, Option<mongodb::bson::oid::ObjectId>> = std::collections::HashMap::new();
        ...
        return Ok(Json(FullDashboardStats {
            pulse: PulseMetrics {
                ...
                integrity: IntegrityMetric {
                    ...
                    target: 100,
                    delta: 0,
                    delta_type: "right".to_string(),
                    status: health_status.to_string(),
                    components: Some(serde_json::json!({
                        "aiModel": { "name": "AI Model", "healthy": health_score >= 85, "score": health_score, "weight": 25 },
                        "backend": { "name": "Backend", "healthy": health_score >= 85, "score": health_score, "weight": 25 },
                        "studentContainers": { "name": "Student Containers", "healthy": health_score >= 85, "score": health_score, "weight": 25 },
                        "adminService": { "name": "Admin Service", "healthy": health_score >= 85, "score": health_score, "weight": 25 }
                    })),
                },
                turnout: PulseMetric {
                    value: 0,
            ...
            charts: ChartsData {
                funnel: FunnelData {
                    total: 0,
                    on_track: FunnelStep { count: 0, percentage: 0 },
                    at_risk: FunnelStep { count: 0, percentage: 0 },
                    disqualified: FunnelStep { count: 0, percentage: 0 },
                },
                integrity_breakdown: IntegrityBreakdown {
                    ...
                    flags: IntegrityFlags {
                        gps_violations: FlagDetail { count: 0, percentage: 0 },
                        device_anomalies: FlagDetail { count: 0, percentage: 0 },
                    },
                },
                system_health: SystemHealthChart {
                    ...
                    summary: SystemHealthSummary {
                        healthy_components: if health_score >= 85 { 4 } else if health_score >= 50 { 2 } else { 0 },
                        total_components: 4,
                    },
                },
                weekly_trends: vec![],
    ...
    let total_checkins = attendance_collection.count_documents(attendance_match.clone()).await? as i64;
    ...
    let mut student_map: std::collections::HashMap<(String, String, String, String), i64> = std::collections::HashMap::new();
    ...
    // Get batch names
    let batch_ids: Vec<mongodb::bson::oid::ObjectId> = session_batch_map
        .values()
        .filter_map(|b| *b)
        .collect();
    
    let mut batch_names: std::collections::HashMap<mongodb::bson::oid::ObjectId, String> = std::collections::HashMap::new();
    ...
    for ((roll_no, name, batch_id_str, _), checkins) in &student_map {
        let expected_checkins = batch_session_counts.get(batch_id_str).copied().unwrap_or(10);
        ...
        let is_low_risk = percentage >= 85;
        let is_medium_risk = percentage >= 75 && percentage < 85;
        let is_high_risk = percentage < 75;
        ...
        if is_medium_risk || is_high_risk {
            ...
            rescue_list.push(RescueItem {
                ...
                trend: if is_low_risk { "up" } else if is_medium_risk { "right" } else { "down" }.to_string(),
            });
        }
    }

    // Sort rescue list by attendance (lowest first) and take top 10
    rescue_list.sort_by(|a, b| a.attendance.cmp(&b.attendance));
    let rescue_count = rescue_list.len() as i64;
    rescue_list.truncate(10);
    ...
    let mut weekly_trends: Vec<WeeklyTrend> = daily_counts
        ...
        .map(|(date, count)| {
            ...
            WeeklyTrend { day: date.clone(), date, rate }
        })
        .collect();
        ...
        if let Some(id) = attendance.id {
            quarantine_list.push(QuarantineItem {
                ...
                flag: match attendance.device_flag.as_ref() {
                    Some(f) => serde_json::to_string(f).unwrap_or_else(|_| "\"Unknown\"".to_string()).trim_matches('"').to_string(),
                    None => "None".to_string(),
                },
    ...
    while batch_cursor.advance().await? {
        ...
        if let Some(batch_id) = batch.id {
            ...
            let batch_session_count = batch_session_counts.get(&batch_id.to_hex()).copied().unwrap_or(0);
    ...
    low_batches.sort_by(|a, b| a.attendance.cmp(&b.attendance));
    ...
    Ok(Json(FullDashboardStats {
        pulse: PulseMetrics {
            eligibility: PulseMetric {
                ...
                delta: 2,
                delta_type: "up".to_string(),
                status: if avg_eligibility >= 85 { "On Track" } else { "At Risk" }.to_string(),
            },
            integrity: IntegrityMetric {
                value: health_score,
                target: 100,
            ...
            quarantine: QuarantineMetric {
                ...
                status: if quarantine_count > 0 { "Critical" } else { "On Track" }.to_string(),
            },
        },
        charts: ChartsData {
            funnel: FunnelData {
                ...
                on_track: FunnelStep {
                    ...
                    percentage: if total_students > 0 { on_track_count * 100 / total_students } else { 0 },
                },
                at_risk: FunnelStep {
                    count: at_risk_count,
                    percentage: if total_students > 0 { at_risk_count * 100 / total_students } else { 0 },
                },
                disqualified: FunnelStep {
                    count: disqualified_count,
                    percentage: if total_students > 0 { disqualified_count * 100 / total_students } else { 0 },
                },
            },
            integrity_breakdown: IntegrityBreakdown {
                total_checkins,
                ...
                flags: IntegrityFlags {
                    gps_violations: FlagDetail {
                        ...
                        percentage: if total_checkins > 0 { gps_flags_count * 100 / total_checkins } else { 0 },
                    },
                    device_anomalies: FlagDetail {
                        count: device_flags_count,
                        percentage: if total_checkins > 0 { device_flags_count * 100 / total_checkins } else { 0 },
                    },
                },
            },
            system_health: SystemHealthChart {
